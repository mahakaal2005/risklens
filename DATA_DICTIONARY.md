...
- **Added 2026-08-28 (generator v0.2.0):** the dataset now scales to 900 merchants x 104 weeks (93,600 rows, up from 220 x 52), and injects two disclosed, seed-reproducible noise sources: (1) a 2% independent null rate on `delivery_evidence_coverage`, `support_ticket_rate`, `average_support_resolution_time_hours`, and `previous_review_outcome`; (2) ~1.5% of rows get an extreme, state-independent anomaly in `refund_rate_30d` or `transaction_volume_30d` (with dependent count/`*_change_30d` columns recomputed to stay internally consistent). Both rates are recorded in `demo_data/synthetic_data_metadata.json`'s `noise_injection` section. See `ml/generate_synthetic_data.py::_inject_missing_data` / `_inject_anomaly_weeks` and `MODEL_CARD.md` for the full rationale.
