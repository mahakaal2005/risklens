...
2. **Review Queue** — five summary metrics (including **SLA breached**), a **Filters** expander, and the case table with a per-row **SLA** column (e.g. "71.8h left" / "⏰ Overdue by 4.2h" / "Closed" / "N/A"). **Click a table row** to select a case, then click **Open case detail**. SLA breach is a simulated in-app indicator only — no real email/SMS notification exists.
